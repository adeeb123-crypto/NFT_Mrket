...
export default function NavComponent() {
  ...
  useEffect(() => {
    ...
    const acc = typeof accounts === "string" ? JSON.parse(accounts) : null;
    setConnectedAcc(acc);
    console.log("acc", acc);
  ...
  async function connectWallet() {
    try {
      if (
        ...
        window.localStorage.setItem("accounts", JSON.stringify(accounts[0]));

        const provider = new ethers.JsonRpcProvider((window as any).ethereum);
        ...
        <div className="hidden lg:block">
          ...
          {isConnected ? (
            <button className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white px-3 py-1 text-[16px] font-bold rounded-md  ">
              {connectedAcc?.slice(0, 6) + "..." + connectedAcc?.slice(36, 42)}
            </button>
          ) : (
            <button
              className="bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 text-white px-3 py-1 text-[16px] font-bold rounded-md  "
